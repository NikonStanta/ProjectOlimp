...
$mysqli->query(
    'UPDATE olimp_reculc SET 
         lock_edit_id='.$lock_edit_id.', 
         lock_edit_dt="'.$lock_edit_dt.'" 
         WHERE lock_edit_id=0 AND id='. $id
);
if ($mysqli->affected_rows < 1) {    // Участник заблокирован или отсутствует
    $res = $mysqli->query('SELECT lock_edit_id, lock_edit_dt FROM olimp_reculc WHERE id='.$id);
    if ($row = $res->fetch_assoc()) {
        die('{"status":"locked", "lock_edit_id":"'.$row['lock_edit_id'].'", "lock_edit_dt":"'.$row['lock_edit_dt'].'"}');
    } else {
        die('{"status":"missing"}');
    }
}
